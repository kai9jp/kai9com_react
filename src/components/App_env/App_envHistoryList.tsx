  ...
  const App_envElements: (JSX.Element | null)[] = App_envs.App_envHistorys.map(App_env => {
    ...
    return (<tr className={`${styles.tablerow} ${(select_record && select_record.modify_count === App_env.modify_count) ? styles.selected : ""}`}
      ...
      <td>{App_env.dir_tmp}</td>
        ...
        <td>{App_env.svn_react_dir}</td>
        <td>{App_env.svn_spring_dir}</td>
        <td>{App_env.svn_scenario_dir}</td>
        <td>{App_env.svn_testdata_dir}</td>
        <td>{App_env.svn_react_url}</td>
        <td>{App_env.svn_spring_url}</td>
        <td>{App_env.svn_scenario_url}</td>
        <td>{App_env.svn_testdata_url}</td>
        <td>{App_env.svn_id}</td>
        <td>{App_env.svn_pw}</td>
            ...
            <div className={`table-responsive ${styles.portlet400}`}>
              <table className="table">
                <thead className="thead-light ">
                  <tr>
                    <th scope="col">#</th>
                    <th scope="col">一時フォルダ</th>
                    <th scope="col">保持日</th>
                    <th scope="col">SVNフォルダ(React)</th>
                    <th scope="col">SVNフォルダ(spring)</th>
                    <th scope="col">SVNフォルダ(ｼﾅﾘｵ)</th>
                    <th scope="col">SVNフォルダ(ﾃｽﾄﾃﾞｰﾀ)</th>
                    <th scope="col">SVN:URL(React)</th>
                    <th scope="col">SVN:URL(spring)</th>
                    <th scope="col">SVN:URL(ｼﾅﾘｵ)</th>
                    <th scope="col">SVN:URL(ﾃｽﾄﾃﾞｰﾀ)</th>
                    <th scope="col">ID</th>
                    <th scope="col">PW</th>
                    <th scope="col">更新者</th>
